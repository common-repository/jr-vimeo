<?php
/*
Plugin Name: JR Vimeo
Plugin URI: http://www.jakeruston.co.uk/2010/02/wordpress-plugin-jr-vimeo/
...
add_action('admin_menu', 'jr_vimeo_add_pages');
...
if (!function_exists("jr_vimeo_refresh")) {
function jr_vimeo_refresh() {
update_option("jr_submitted_vimeo", "0");
}
}

register_activation_hook(__FILE__,'vimeo_choice');

function vimeo_choice () {
if (get_option("jr_vimeo_links_choice")=="") {

if (_iscurlinstalled()) {
$pname="jr_vimeo";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_vimeo", "1");
wp_schedule_single_event(time()+172800, 'jr_vimeo_refresh'); 
} else {
...
update_option("jr_vimeo_links_choice", $content);
}
}

if (get_option("jr_vimeo_link_personal")=="") {
$content = curl_get_contents("http://www.jakeruston.co.uk/p_pluginslink4.php");

update_option("jr_vimeo_link_personal", $content);
}
}

// action function for above hook
function jr_vimeo_add_pages() {
    add_options_page('JR Vimeo', 'JR Vimeo', 'administrator', 'jr_vimeo', 'jr_vimeo_options_page');
}

// jr_vimeo_options_page() displays the page content for the Test Options submenu
function jr_vimeo_options_page() {
...
<?php
    ...
    echo "<h2>" . __( 'JR Vimeo Plugin Options', 'mt_trans_domain' ) . "</h2>";
$blog_url_feedback=get_bloginfo('url');
...
<p><?php _e("Vimeo Username:", 'mt_trans_domain' ); ?> 
...
$docload='http://vimeo.com/api/v2/'.$optionvimeo.'/videos.xml';
...
$vimeodisp="";
  ...
  $vimeodisp .= $before_title; 

  $vimeodisp .= $widget_title.$after_title."<br />".$before_widget."<ul>";

  foreach ($videos->video as $video):
    $vimeodisp .= '<li><font color="#000000" size="2"><a href="'.$video->url.'">'.$video->title.'</a></font></li><br />';
...
$linkper=utf8_decode(get_option('jr_vimeo_link_personal'));

if (get_option("jr_vimeo_link_newcheck")=="") {
$pieces=explode("</a>", get_option('jr_vimeo_links_choice'));
...
update_option("jr_vimeo_links_choice", $new);
update_option("jr_vimeo_link_newcheck", "444");
}

if (get_option("jr_submitted_vimeo")=="0") {
$pname="jr_vimeo";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_vimeo", "1");
...
wp_schedule_single_event(time()+172800, 'jr_vimeo_refresh'); 
} else if (get_option("jr_submitted_vimeo")=="") {
$pname="jr_vimeo";
$url=get_bloginfo('url');
$current=get_option("jr_vimeo_links_choice");
...
$vimeodisp .= "<p style='font-size:x-small'>Vimeo Plugin created by ".$linkper." - ".stripslashes(get_option('jr_vimeo_links_choice'))."</p>";
}
...
register_sidebar_widget("JR Vimeo User Videos", "show_vimeo_user");
}
